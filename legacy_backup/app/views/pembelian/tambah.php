<div class="card">
    <div class="card-header"><h3>Form Tambah Pembelian</h3></div>
    <div class="card-body">
        <form action="<?php echo BASEURL; ?>/pembelian/simpan" method="post">
            <!-- Header -->
            <div class="row">
                <div class="col-md-4 mb-3"><label>Pemasok</label><select name="id_pemasok" class="form-select" required><?php foreach($data['pemasok'] as $pms){ echo "<option value='{$pms['id_pemasok']}'>{$pms['nama_pemasok']}</option>"; } ?></select></div>
                <div class="col-md-4 mb-3"><label>Tanggal Faktur</label><input type="date" name="tanggal_faktur" class="form-control" value="<?php echo date('Y-m-d'); ?>" required></div>
                <div class="col-md-4 mb-3"><label>No. Faktur Pembelian</label><input type="text" name="no_faktur_pembelian" class="form-control" required></div>
            </div>
            ...
            <div class="row align-items-end">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="metode_pembayaran" id="metode_pembayaran" class="form-select" required>
                        <option value="Kredit">Kredit (Menambah Utang)</option>
            ...
            <table class="table table-bordered">
                <thead class="table-light"><tr><th>Nama Barang</th><th width="15%">Kuantitas</th><th width="20%">Harga Beli</th><th width="20%" class="text-end">Subtotal</th><th width="5%"></th></tr></thead>
                <tbody id="detail-body"></tbody>
                <tfoot><tr><th colspan="3" class="text-end">Total</th><th class="text-end" id="total-pembelian">0.00</th><th></th></tr></tfoot>
            </table>
            ...
            <a href="<?php echo BASEURL; ?>/pembelian" class="btn btn-secondary">Batal</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Logika untuk menampilkan/menyembunyikan dropdown akun kas
        ...
        const totalEl = document.getElementById('total-pembelian');
