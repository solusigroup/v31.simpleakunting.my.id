...
class Jurnal_model {
    private $tabel_header = 'jurnal_umum';
    private $tabel_detail = 'jurnal_detail';
    private $db;
    ...
    public function getAllJurnal()
    {
        $query = "SELECT 
                    ...
                    pjl.id_penjualan,
                    pbl.id_pembelian
                  FROM {$this->tabel_header} ju
                  LEFT JOIN {$this->tabel_detail} jd ON ju.id_jurnal = jd.id_jurnal
    ...
    public function simpanJurnal($data)
    {
        ...
        try {
            $queryHeader = "INSERT INTO {$this->tabel_header} (no_transaksi, tanggal, deskripsi, sumber_jurnal) 
            ...
            $queryDetail = "INSERT INTO {$this->tabel_detail} (id_jurnal, kode_akun, debit, kredit)
    ...
    public function updateJurnal($data)
    {
        ...
        try {
            $queryHeader = "UPDATE {$this->tabel_header} SET no_transaksi = :no_transaksi, tanggal = :tanggal, deskripsi = :deskripsi WHERE id_jurnal = :id_jurnal";
            $this->db->query($queryHeader);
            $this->db->bind('no_transaksi', $data['no_transaksi']);
            $this->db->bind('tanggal', $data['tanggal']);
            $this->db->bind('deskripsi', $data['deskripsi']);
            ...
            $this->db->execute();
            $this->db->query("DELETE FROM {$this->tabel_detail} WHERE id_jurnal = :id_jurnal");
            $this->db->bind('id_jurnal', $data['id_jurnal']);
            $this->db->execute();
            $queryDetail = "INSERT INTO {$this->tabel_detail} (id_jurnal, kode_akun, debit, kredit) VALUES (:id_jurnal, :kode_akun, :debit, :kredit)";
            foreach ($data['details'] as $detail) {
                if(!empty($detail['kode_akun']) && ($detail['debit'] > 0 || $detail['kredit'] > 0)){
                    $this->db->query($queryDetail);
                    $this->db->bind('id_jurnal', $data['id_jurnal']);
    ...
    public function hapusJurnal($id, $force = false)
    {
        ...
        $query = "DELETE FROM {$this->tabel_header} WHERE id_jurnal = :id_jurnal";
        $this->db->query($query);
    ...
    {
        $this->db->query("SELECT is_locked FROM {$this->tabel_header} WHERE id_jurnal = :id");
        $this->db->bind('id', $id);
    ...
    public function getNeracaSaldo($tanggal_selesai)
    {
        $sql = "
            SELECT 
                a.kode_akun, a.nama_akun, a.posisi_saldo_normal, a.saldo_awal,
                COALESCE(SUM(jd.debit), 0) as total_debit,
                COALESCE(SUM(jd.kredit), 0) as total_kredit
            FROM 
                akun a
            LEFT JOIN 
                (jurnal_detail jd JOIN jurnal_umum ju ON jd.id_jurnal = ju.id_jurnal)
                ON a.kode_akun = jd.kode_akun AND ju.tanggal <= :tanggal_selesai
            WHERE 
                a.tipe_akun = 'Detail'
    ...
    public function getLabaRugi($tgl_mulai_1, $tgl_selesai_1, $tgl_mulai_2 = null, $tgl_selesai_2 = null)
    {
        $sql = "
            SELECT 
                ...
                SUM(CASE WHEN ju.tanggal BETWEEN :tgl_mulai_1 AND :tgl_selesai_1 THEN jd.debit ELSE 0 END) as total_debit_1,
                SUM(CASE WHEN ju.tanggal BETWEEN :tgl_mulai_1 AND :tgl_selesai_1 THEN jd.kredit ELSE 0 END) as total_kredit_1,
                SUM(CASE WHEN ju.tanggal BETWEEN :tgl_mulai_2 AND :tgl_selesai_2 THEN jd.debit ELSE 0 END) as total_debit_2,
                SUM(CASE WHEN ju.tanggal BETWEEN :tgl_mulai_2 AND :tgl_selesai_2 THEN jd.kredit ELSE 0 END) as total_kredit_2
            FROM akun a
            LEFT JOIN jurnal_detail jd ON a.kode_akun = jd.kode_akun
            LEFT JOIN jurnal_umum ju ON jd.id_jurnal = ju.id_jurnal
            WHERE 
                a.tipe_akun = 'Detail' AND (a.kode_akun LIKE '4-%' OR a.kode_akun LIKE '5-%' OR a.kode_akun LIKE '6-%' OR a.kode_akun LIKE '8-%')
            GROUP BY a.kode_akun
            ORDER BY a.kode_akun ASC";
        $this->db->query($sql);
    ...
    public function getNeracaLajur($tanggal_selesai)
    {
        ...
        foreach ($neracaSaldo as $akun) {
            ...
            if ($kode_awal == '4' || $kode_awal == '5' || $kode_awal == '6' || $kode_awal == '8') {
    ...
    {
        $processData = function($neracaLajur) {
            $result = ['aset' => [], 'kewajiban' => [], 'modal' => [], 'total_aset' => 0, 'total_kewajiban' => 0, 'total_modal' => 0];
            ...
            foreach ($neracaLajur['data'] as $akun) {
                if ($akun['neraca_debit'] > 0 || $akun['neraca_kredit'] > 0) {
                    ...
                    $kode_awal = substr($akun['kode_akun'], 0, 1);
                    if ($kode_awal == '1') { $result['aset'][$akun['kode_akun']] = $item; $result['total_aset'] += $item['total']; } 
                    elseif ($kode_awal == '2') { $result['kewajiban'][$akun['kode_akun']] = $item; $result['total_kewajiban'] += $item['total']; } 
                    elseif ($kode_awal == '3') { $result['modal'][$akun['kode_akun']] = $item; $result['total_modal'] += $item['total']; }
                }
            }
            $labaRugiBersih = $neracaLajur['laba_rugi_bersih'];
            $result['modal']['laba_rugi'] = ['kode_akun' => '', 'nama_akun' => 'Laba (Rugi) Periode Berjalan', 'total' => $labaRugiBersih];
            $result['total_modal'] += $labaRugiBersih;
            return $result;
        };
        $laporan_1 = $processData($this->getNeracaLajur($tgl_selesai_1));
        $laporan_2 = $tgl_selesai_2 ? $processData($this->getNeracaLajur($tgl_selesai_2)) : null;
