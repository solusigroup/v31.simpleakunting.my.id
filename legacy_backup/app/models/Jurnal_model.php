...
class Jurnal_model {
    ...
    public function getLabaRugi($tgl_mulai_1, $tgl_selesai_1, $tgl_mulai_2 = null, $tgl_selesai_2 = null) {
        $sql = "
            ...
            LEFT JOIN (
                ...
                WHERE ju.tanggal BETWEEN :tgl_mulai_1 AND :tgl_selesai_1 GROUP BY jd.kode_akun
            ) trx1 ON a.kode_akun = trx1.kode_akun
            LEFT JOIN (
                SELECT jd.kode_akun, SUM(jd.debit) AS total_debit, SUM(jd.kredit) AS total_kredit
                FROM jurnal_detail jd JOIN jurnal_umum ju ON jd.id_jurnal = ju.id_jurnal
                WHERE ju.tanggal BETWEEN :tgl_mulai_2 AND :tgl_selesai_2 GROUP BY jd.kode_akun
            ) trx2 ON a.kode_akun = trx2.kode_akun
            WHERE a.tipe_akun = 'Detail' AND SUBSTR(a.kode_akun, 1, 1) IN ('4', '5', '6', '7', '8')
        ...
        foreach ($results as $row) {
            ...
            if ($saldo1 != 0 || $saldo2 != 0) {
                ...
                if ($row['posisi_saldo_normal'] == 'Kredit') {
    ...
    public function getArusKas($tanggal_mulai, $tanggal_selesai) {
        $this->db->query("SELECT akun_piutang_default, akun_utang_default FROM perusahaan WHERE id = 1");
        $pengaturan = $this->db->single();
        $akun_piutang = $pengaturan['akun_piutang_default'];
        $akun_utang = $pengaturan['akun_utang_default'];
        // Asumsi Akun Persediaan. Ini bisa dipindahkan ke pengaturan di masa depan.
        $akun_persediaan = '1.2.01.01'; 
        ...
        $saldoAkun = [];
        foreach ($neracaSaldoAwal as $akun) { $saldoAkun[$akun['kode_akun']]['awal'] = $akun['debit'] - $akun['kredit']; }
        foreach ($neracaSaldoAkhir as $akun) { $saldoAkun[$akun['kode_akun']]['akhir'] = $akun['debit'] - $akun['kredit']; }
        
        $perubahanPiutang = ($saldoAkun[$akun_piutang]['akhir'] ?? 0) - ($saldoAkun[$akun_piutang]['awal'] ?? 0);
        $perubahanPersediaan = ($saldoAkun[$akun_persediaan]['akhir'] ?? 0) - ($saldoAkun[$akun_persediaan]['awal'] ?? 0);
        $perubahanUtang = ($saldoAkun[$akun_utang]['akhir'] ?? 0) - ($saldoAkun[$akun_utang]['awal'] ?? 0);

        $kasAwal = 0; $kasAkhir = 0;
        // **PERBAIKAN: Gunakan grup_akun untuk mengidentifikasi kas & bank**
        foreach ($neracaSaldoAwal as $akun) { if (trim(strtolower($akun['grup_akun'] ?? '')) == 'kas & bank') $kasAwal += $akun['debit'] - $akun['kredit']; }
        foreach ($neracaSaldoAkhir as $akun) { if (trim(strtolower($akun['grup_akun'] ?? '')) == 'kas & bank') $kasAkhir += $akun['debit'] - $akun['kredit']; }
    ...
    /**
     * FUNGSI BARU: Menghitung data untuk Neraca Lajur 16 kolom yang lengkap.
     */
    public function getNeracaLajurLengkap($tanggal_mulai, $tanggal_selesai)
    {
        $this->db->query("SELECT kode_akun, nama_akun, saldo_awal, posisi_saldo_normal FROM akun WHERE tipe_akun = 'Detail' ORDER BY kode_akun ASC");
        $akun_list = $this->db->resultSet();

        $this->db->query("SELECT jd.kode_akun, ju.tanggal, ju.sumber_jurnal, jd.debit, jd.kredit 
                          FROM jurnal_detail jd
                          JOIN jurnal_umum ju ON jd.id_jurnal = ju.id_jurnal
                          WHERE ju.tanggal <= :tanggal_selesai");
        $this->db->bind('tanggal_selesai', $tanggal_selesai);
